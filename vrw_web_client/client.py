...
class VRWClient(object):
    ...
    # 検索用のフィルター追加
    def filter(self, key, value, mode:QueryType = QueryType.EQ):
        """
        kwargs: dict
        検索条件を指定して検索
        """
    ...
    def _get_item(self, path=""):
        ...
        res =  api.get(path).get("Item", {})
        return schema.parse(res)
    
    def _get_items_list(self, path=""):
        path = self.set_url(path)
        res = api.get(path, self.filter_params).get("Items", [])
        return [schema.parse(item) for item in res]
