...
def serialize_data(data):
    change_dict_key(data, "type", "series")
    change_dict_key(data, "id", "NextSpaceFlight")

    data["NextSpaceFlight"] = "a" + data["NextSpaceFlight"]
...
class SampleTest(unittest.TestCase):
    def test(self):
        ...
        item = ReuseVehicle()
        res_launch = item.batch_put(items)
        print(res_launch)
        res_launch["results"] = []
        ...
        for i, target in enumerate(res_launch["results"]):
            if images[i] is not None:
                ...
                # 画像を保存
                image = Image()
                res_image = image.put(image_data)
                print("res_image:", str(res_image))
                res_image_rel = item.put_ref_item(target["Item"], res_image)
